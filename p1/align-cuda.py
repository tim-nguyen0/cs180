...
def vectorized_calculate_offset_ncc_chunked(ref_in: np.array, child: np.array, max_offset: int=5, known_offset: tuple=(0,0)) -> tuple:
    ...
    ref_windows = swv(ref, template_size)
    W = ref_windows.shape[0]

    means  = xp.empty((W, W), dtype=xp.float32)
    sum_sq = xp.empty((W, W), dtype=xp.float32)
    cross  = xp.empty((W, W), dtype=xp.float32)
    for i in range(W):                                    
        row = ref_windows[i]                                
        means[i]  = xp.sum(row, axis=(-2,-1)) / n
        sum_sq[i] = xp.einsum('jhw,jhw->j', row, row)
        cross[i]  = xp.einsum('jhw,hw->j', row, child_patch_normed)

    variances = sum_sq / n - means**2
    sigmas = xp.sqrt(xp.maximum(variances, 1e-12))
    ncc_map = cross / (n * sigmas)  
    max_y, max_x = xp.unravel_index(xp.argmax(ncc_map), ncc_map.shape)   
